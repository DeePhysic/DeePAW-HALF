...
contains
  ...
  subroutine build_veff(charge,potcars,crystal,use_pbe,veff,e_hartree,e_xc,e_xc_potential,vxc_out)
    ...
    real(dp),allocatable::charge_c(:),local_r(:),core_r(:),vxc(:),m2local(:,:)
    ...
    allocate(m2local(1000,nt))
    do it=1,nt
      call vasp_local_spline_second(potcars(it)%psp_local,potcars(it)%psp_gmax,m2local(:,it))
    end do
    ...
    do i1=0,charge%shape(1)-1
      ...
      do i2=0,charge%shape(2)-1
        ...
        do i3=0,charge%shape(3)-1
          ...
          ion0=0
          do it=1,nt
            radial=vasp_local_spline_eval(potcars(it)%psp_local,m2local(:,it),potcars(it)%psp_gmax,gn)
            ...
            if(potcars(it)%has_core.and.gn<=potcars(it)%psp_gmax-3*potcars(it)%psp_gmax/1000) &
              core_radial=vasp_four_point_eval(potcars(it)%pspcor,potcars(it)%psp_gmax,gn)
