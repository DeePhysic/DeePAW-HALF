...
| Analytic forces | not present in HAPPY | `half_energy.F90`, `half_local_forces.F90`, `half_forces.F90`, `half_uspp.F90` | in progress: analytic Ewald, reciprocal local, NLCC/`FORCOR`, generalized PAW `(D-epsilon Q)`, and augmentation `dD/dR` terms implemented and derivative-tested; fixed-density Harris correction and complete CLI/GPU integration remain before production use |
| Production `--forces` contract | n/a | `half_cli.F90` | reserved exclusively for the complete analytic PAW/Harris force and must never displace atoms; currently refuses execution until `FORDEP/FORHAR` validation is complete |
| Finite-difference force oracle | `total_energy.py` | `half_cli.F90 --finite-difference-force-check` | explicitly named developer validation only; Si parity within 2.3e-9 eV/Angstrom, never the production force algorithm |
