...
| Analytic forces | not present in HAPPY | `half_energy.F90`, `half_local_forces.F90`, `half_forces.F90`, `half_uspp.F90` | implemented: Ewald, local, NLCC/`FORCOR`, generalized PAW projector `(D-epsilon Q)`, POTCAR augmentation density, augmentation `dD/dR`, and frozen-smooth-density/core-XC Harris response are wired into the standalone CLI; Si/HfO2 selected components agree with the same HALF energy finite difference within 0.00175/0.00307 eV/Angstrom, while exact VASP onsite parity remains active |
