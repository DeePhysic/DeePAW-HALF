...
| Analytic forces | not present in HAPPY | `half_energy.F90`, `half_local_forces.F90`, `half_forces.F90`, `half_uspp.F90`, `half_potcar_interp.F90` | implemented: Ewald, local, NLCC/`FORCOR`, generalized PAW projector `(D-epsilon Q)`, POTCAR augmentation density, augmentation `dD/dR`, and the full frozen-density Hartree+XC Harris response contracted with POTCAR `PSPRHO`; direct-grid `dD/dR` agrees with the displacement oracle within `2.8e-9 eV/Angstrom`, and strict frozen-density VASP force MAEs are `6.99e-6/1.01e-3 eV/Angstrom` for Si/HfO2 |
