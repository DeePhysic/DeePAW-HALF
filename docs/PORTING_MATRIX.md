...
| Potential-dependent MIMIC_US D | `uspp_dij.py` | `half_uspp.F90`, `half_cuda_uspp.cuf` | done on CPU/CUDA: VASP-style direct FFT-grid QDEP/SETDIJ, POTCAR-log-grid `QPAW(i,j,L)` reconstruction, atom-dependent D, and legacy shell quadrature as an A/B switch |
...
| Occupations, EIGENVAL, Ewald and energy | `occupations.py`, `ewald.py`, `total_energy.py` | `half_energy.F90`, `half_paw_atomic_energy.F90`, `half_cli.F90` | done on CPU/CUDA; zero/finite-T occupations, reconstructed and reference-EIGENVAL paths, and POTCAR-derived spherical atomic PAW double counting; exact VASP onsite PAW parity remains active |
| Analytic forces | not present in HAPPY | `half_energy.F90`, `half_local_forces.F90`, `half_forces.F90`, `half_uspp.F90` | implemented: Ewald, local, NLCC/`FORCOR`, generalized PAW projector `(D-epsilon Q)`, POTCAR augmentation density, augmentation `dD/dR`, and frozen-smooth-density/core-XC Harris response are wired into the standalone CLI; direct-grid `dD/dR` agrees with the independent displacement oracle within `2.8e-9 eV/Angstrom` for Si/HfO2, while remaining HfO2 VASP total-force parity is isolated to other one-centre/output-density terms |
| Production `--forces` contract | n/a | `half_cli.F90` | implemented as the standalone analytic PAW/Harris path and never displaces atoms; the force log separates projector and POTCAR augmentation-density contributions |
| Finite-difference force oracle | `total_energy.py` | `half-force-check` and validation records | developer validation only; never the production force algorithm |
...
| MPI k-point distribution | n/a | `half_parallel.F90`, `half_cli.F90` | done for CPU bands, energy and the explicitly named finite-difference validation oracle; cyclic distribution, rank-0 output and collective reconstruction; 3.903x on 4 ranks for the 36-k-point HfO2 case |
