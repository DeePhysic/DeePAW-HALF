...
| Analytic forces | not present in HAPPY | `half_energy.F90`, `half_local_forces.F90`, `half_forces.F90` | in progress: analytic Ewald, reciprocal local, NLCC/`FORCOR`, and generalized PAW `(D-epsilon Q)` terms implemented and derivative-tested; augmentation and fixed-density Harris corrections remain before production use |
