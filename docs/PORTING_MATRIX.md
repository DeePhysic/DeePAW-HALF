...
| Analytic forces | not present in HAPPY | `half_energy.F90`, `half_local_forces.F90`, `half_forces.F90`, `half_uspp.F90` | active: Ewald, smooth-density local, NLCC/`FORCOR`, generalized PAW projector `(D-epsilon Q)`, POTCAR augmentation-density reconstruction, augmentation `dD/dR`, and fixed-density Harris response are wired into the standalone CLI; `dD/dR` is derivative-tested and total-force finite-difference validation remains active |
